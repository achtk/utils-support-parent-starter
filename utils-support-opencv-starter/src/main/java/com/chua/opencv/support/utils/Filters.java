...
public class Filters {
	...
	public static Mat GuidedImageFilterColor(Mat origI, Mat p, int r, double eps, double s, int depth) {
		...
		r_sub = r / s;
		meanIR = boxfilter(Isubchannels.get(0), (int) r_sub);
		meanIG = boxfilter(Isubchannels.get(1), (int) r_sub);
		meanIB = boxfilter(Isubchannels.get(2), (int) r_sub);
		Mat varIRr = new Mat();
		Mat varIRg = new Mat();
		Mat varIRb = new Mat();
		Mat varIGg = new Mat();
		...
		Core.subtract(boxfilter(Isubchannels.get(0).mul(Isubchannels.get(0)), (int) r_sub), 
				meanIR.mul(meanIR), temp1);
		Core.add(temp1, new Scalar(eps), varIRr);
		Core.subtract(boxfilter(Isubchannels.get(0).mul(Isubchannels.get(1)), (int) r_sub), 
				meanIR.mul(meanIG), varIRg);
		Core.subtract(boxfilter(Isubchannels.get(0).mul(Isubchannels.get(2)), (int) r_sub), 
				meanIR.mul(meanIB), varIRb);
		Core.subtract(boxfilter(Isubchannels.get(1).mul(Isubchannels.get(1)), (int) r_sub), 
				meanIG.mul(meanIG), temp1);
		Core.add(temp1, new Scalar(eps), varIGg);
		Core.subtract(boxfilter(Isubchannels.get(1).mul(Isubchannels.get(2)), (int) r_sub), 
				meanIG.mul(meanIB), varIGb);
		Core.subtract(boxfilter(Isubchannels.get(2).mul(Isubchannels.get(2)), (int) r_sub), 
				meanIB.mul(meanIB), temp1);
		Core.add(temp1, new Scalar(eps), varIBb);

		// Inverse of Sigma + eps * I
		Core.subtract(varIGg.mul(varIBb), varIGb.mul(varIGb), invrr);
		Core.subtract(varIGb.mul(varIRb), varIRg.mul(varIBb), invrg);
		Core.subtract(varIRg.mul(varIGb), varIGg.mul(varIRb), invrb);
		Core.subtract(varIRr.mul(varIBb), varIRb.mul(varIRb), invgg);
		Core.subtract(varIRb.mul(varIRg), varIRr.mul(varIGb), invgb);
		Core.subtract(varIRr.mul(varIGg), varIRg.mul(varIRg), invbb);

		Mat covDet = new Mat();
		Core.add(invrr.mul(varIRr), invrg.mul(varIRg), temp1);
		Core.add(temp1, invrb.mul(varIRb), covDet);
		...
		Mat result = new Mat();
		if (p.channels() == 1) {
			result = filterSingleChannel(p2, s, Isubchannels, Ichannels, meanIR, meanIG, meanIB, invrr, invrg,
					invrb, invgg, invgb, invbb, r_sub);
		} else {
			ArrayList<Mat> pc = new ArrayList<>();
			Core.split(p2, pc);
			for (int i = 0; i < pc.size(); i++) {
				pc.set(i, filterSingleChannel(pc.get(i), s, Isubchannels, Ichannels, meanIR, meanIG, meanIB, invrr,
