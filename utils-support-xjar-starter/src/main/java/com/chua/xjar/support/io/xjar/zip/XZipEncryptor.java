package com.chua.xjar.support.io.xjar.zip;
...
public class XZipEncryptor extends XEntryEncryptor<ZipArchiveEntry> implements XEncryptor {
    ...
    public XZipEncryptor(XEncryptor xEncryptor, XEntryFilter<ZipArchiveEntry> filter, XEntryFilter<ZipArchiveEntry> dependsFilter) {
        this(xEncryptor, Deflater.DEFAULT_COMPRESSION, filter, dependsFilter);
    }

    public XZipEncryptor(XEncryptor xEncryptor, int level) {
        this(xEncryptor, level, null, null);
    }

    public XZipEncryptor(XEncryptor xEncryptor, int level, XEntryFilter<ZipArchiveEntry> filter, XEntryFilter<ZipArchiveEntry> dependsFilter) {
