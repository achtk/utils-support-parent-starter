...
import com.chua.xjar.support.io.xjar.jar.XJarAllEntryFilter;
...
public class XBootEncryptor extends XEntryEncryptor<JarArchiveEntry> implements XEncryptor, XConstants {
...

    {
        ...
        map.put(jarLauncher, "com.chua.xjar.support.io.xjar.boot.XJarLauncher");
        map.put(warLauncher, "com.chua.xjar.support.io.xjar.boot.XWarLauncher");
        map.put(extLauncher, "com.chua.xjar.support.io.xjar.boot.XExtLauncher");
    }

    private final int level;

    public XBootEncryptor(XEncryptor xEncryptor) {
        this(xEncryptor, new XJarAllEntryFilter(), null);
    }

    public XBootEncryptor(XEncryptor xEncryptor, XEntryFilter<JarArchiveEntry> filter, XEntryFilter<JarArchiveEntry> dependsFilter) {
        this(xEncryptor, Deflater.DEFAULT_COMPRESSION, filter, dependsFilter);
    }

    public XBootEncryptor(XEncryptor xEncryptor, int level) {
        this(xEncryptor, level, new XJarAllEntryFilter(), null);
    }

    public XBootEncryptor(XEncryptor xEncryptor, int level, XEntryFilter<JarArchiveEntry> filter, XEntryFilter<JarArchiveEntry> dependsFilter) {
    ...
    public void encrypt(XKey key, InputStream in, OutputStream out) throws IOException {
        ...
        try {
            ...
            XJarEncryptor xJarEncryptor = new XJarEncryptor(xEncryptor, level, filter, dependsFilter);
            ...
            while ((entry = zis.getNextJarEntry()) != null) {
                ...
                // BOOT-INF/lib/**
                    ...
                    XBootLibArchiveEntry xBootJarArchiveEntry = new XBootLibArchiveEntry(entry);
                    boolean filtered = filtrate(xBootJarArchiveEntry);
