...
public class AudioFeaturizer {
  ...
  public static NDArray featurize(NDManager manager, float[] floatArray) {
    ...
    float rmsDb = AudioAsrUtils.rmsDb(samples);
    // 返回以分贝为单位的音频均方根能量
    System.out.println("音频均方根能量: " + rmsDb);

    // 提取特征前将音频归一化至-20 dB(以分贝为单位)
    float target_dB = -20f;
    samples = AudioAsrUtils.normalize(samples, target_dB);
    ...
    float window_ms = 20f;
    // 用快速傅里叶变换计算线性谱图
    NDArray specgram = AudioAsrUtils.linearSpecgram(manager, samples, stride_ms, window_ms);
