 ...
 */
package com.chua.agent.support.json.parser;

import com.chua.agent.support.json.*;
import com.chua.agent.support.json.annotation.JSONCreator;
import com.chua.agent.support.json.annotation.JSONField;
import com.chua.agent.support.json.annotation.JSONType;
import com.chua.agent.support.json.asm.ClassReader;
import com.chua.agent.support.json.asm.TypeCollector;
import com.chua.agent.support.json.parser.deserializer.*;
import com.chua.agent.support.json.serializer.*;
import com.chua.agent.support.json.spi.Module;
import com.chua.agent.support.json.util.IdentityHashMap;
import com.chua.agent.support.json.util.ServiceLoader;
...
public class ParserConfig {
    ...
    public Class<?> checkAutoType(String typeName, Class<?> expectClass, int features) {
        ...
        final long h1 = (TypeUtils.fnv1a_64_magic_hashcode ^ className.charAt(0)) * TypeUtils.fnv1a_64_magic_prime;
        if (h1 == 0xaf64164c86024f1aL) { // [
            throw new JSONException("autoType is not support. " + typeName);
        }

        if ((h1 ^ className.charAt(className.length() - 1)) * TypeUtils.fnv1a_64_magic_prime == 0x9198507b5af98f0L) {
            throw new JSONException("autoType is not support. " + typeName);
        }

        final long h3 = (((((TypeUtils.fnv1a_64_magic_hashcode ^ className.charAt(0))
                * TypeUtils.fnv1a_64_magic_prime)
                ^ className.charAt(1))
                * TypeUtils.fnv1a_64_magic_prime)
                ^ className.charAt(2))
                * TypeUtils.fnv1a_64_magic_prime;
