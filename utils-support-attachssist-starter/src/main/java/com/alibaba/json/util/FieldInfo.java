...
public class FieldInfo implements Comparable<FieldInfo> {
    ...
    public FieldInfo(String name, // 
        ...
        if (field != null) {
            com.alibaba.json.util.TypeUtils.setAccessible(field);
        }

        this.label = "";
        fieldAnnotation = field == null ? null : com.alibaba.json.util.TypeUtils.getAnnotation(field, JSONField.class);
    ...
    public FieldInfo(String name, //
        ...
        if (field != null) {
            ...
            fieldAccess = ((modifiers & Modifier.PUBLIC) != 0 || method == null);
            fieldTransient = Modifier.isTransient(modifiers)
                    || com.alibaba.json.util.TypeUtils.isTransient(method);
        } else {
            fieldAccess = false;
            fieldTransient = com.alibaba.json.util.TypeUtils.isTransient(method);
        }
        ...
        if (method != null) {
            com.alibaba.json.util.TypeUtils.setAccessible(method);
        }
        ...
        if (clazz != null && fieldClass == Object.class && fieldType instanceof TypeVariable) {
            ...
            if (genericFieldType != null) {
                this.fieldClass = com.alibaba.json.util.TypeUtils.getClass(genericFieldType);
        ...
        if (!(fieldType instanceof Class)) {
            ...
            if (genericFieldType != fieldType) {
                if (genericFieldType instanceof ParameterizedType) {
                    fieldClass = com.alibaba.json.util.TypeUtils.getClass(genericFieldType);
    ...
    public static Type getFieldType(final Class<?> clazz, final Type type, Type fieldType, Map<TypeVariable, Type> genericInfo) {
        ...
        if (fieldType instanceof GenericArrayType) {
            ...
            if (componentType != componentTypeX) {
                Type fieldTypeX = Array.newInstance(com.alibaba.json.util.TypeUtils.getClass(componentTypeX), 0).getClass();
                return fieldTypeX;
            }
        ...
        if (!com.alibaba.json.util.TypeUtils.isGenericParamType(type)) {
            return fieldType;
        }

        if (fieldType instanceof TypeVariable) {
            ParameterizedType paramType = (ParameterizedType) com.alibaba.json.util.TypeUtils.getGenericParamType(type);
            Class<?> parameterizedClass = com.alibaba.json.util.TypeUtils.getClass(paramType);
        ...
        if (fieldType instanceof ParameterizedType) {
            ...
            if (changed) {
                fieldType = TypeReference.intern(
                        new com.alibaba.json.util.ParameterizedTypeImpl(arguments, parameterizedFieldType.getOwnerType(),
            ...
            if (typeArg instanceof ParameterizedType) {
                ...
                if (p_changed) {
                    typeArgs[i] = TypeReference.intern(
                            new com.alibaba.json.util.ParameterizedTypeImpl(p_typeArg_args, p_typeArg.getOwnerType(), p_typeArg.getRawType())
                    );
                    changed = true;
                }
    ...
    private long nameHashCode64(String name, JSONField annotation)
    {
        if (annotation != null && annotation.name().length() != 0) {
            return com.alibaba.json.util.TypeUtils.fnv1a_64_lower(name);
        }
        return com.alibaba.json.util.TypeUtils.fnv1a_64_extract(name);
    }
    ...
    public <T extends Annotation> T getAnnation(Class<T> annotationClass) {
        ...
        T annotatition = null;
        if (method != null) {
            annotatition = com.alibaba.json.util.TypeUtils.getAnnotation(method, annotationClass);
        }

        if (annotatition == null && field != null) {
            annotatition = com.alibaba.json.util.TypeUtils.getAnnotation(field, annotationClass);
        }
