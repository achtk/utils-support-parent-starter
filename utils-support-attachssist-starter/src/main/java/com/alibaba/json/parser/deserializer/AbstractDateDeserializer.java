    ...
    public <T> T deserialze(DefaultJSONParser parser, Type clazz, Object fieldName, String format, int features) {
        ...
        if (lexer.token() == JSONToken.LITERAL_INT) {
            ...
            if (format != null) {
                ...
                if (com.alibaba.json.JSON.defaultTimeZone != null) {
                ...
                if (val == null && com.alibaba.json.JSON.defaultLocale == Locale.CHINA) {
                    try {
                ...
                if (val == null) {
                    if (format.equals("yyyy-MM-dd'T'HH:mm:ss.SSS") //
                            && strVal.length() == 19) {
                        try {
                            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", com.alibaba.json.JSON.defaultLocale);
                            df.setTimeZone(com.alibaba.json.JSON.defaultTimeZone);
                            val = df.parse(strVal);
