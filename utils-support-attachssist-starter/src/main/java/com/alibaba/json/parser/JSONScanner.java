...
public final class JSONScanner extends JSONLexerBase {
    ...
    public JSONScanner(String input){
        this(input, com.alibaba.json.JSON.DEFAULT_PARSER_FEATURE);
    }
    ...
    public byte[] bytesValue() {
        if (token == JSONToken.HEX) {
            ...
            if (len % 2 != 0) {
                throw new com.alibaba.json.JSONException("illegal state. " + len);
            }
        ...
        if (!hasSpecial) {
            return com.alibaba.json.util.IOUtils.decodeBase64(text, np + 1, sp);
        } else {
        ...
        if (chLocal >= '0' && chLocal <= '9') {
            ...
            for (;;) {
                ...
                if (chLocal >= '0' && chLocal <= '9') {
                    int value_10 = value * 10;
                    if (value_10 < value) {
                        throw new com.alibaba.json.JSONException("parseInt error : "
        ...
        for (; i < text.length(); ++i) {
            ...
            if (ch == '\\') {
                if (i < len - 1) {
                    ...
                    this.ch = ch;
                    this.bp = i;
                    throw new com.alibaba.json.JSONException("illegal str, " + info());
                }
            } else if (ch == '"') {
                quote = !quote;
            } else if (ch == '{') {
...
//        nextToken();
        ...
        for (int i = 0; i < index; ++i) {
            ...
            if (ch == '"' || ch == '\'') {
                ...
                if (ch == ',') {
                    ...
                    throw new com.alibaba.json.JSONException("illegal json.");
                }
            } else if (ch == '{') {
                next();
    ...
    public int seekObjectToField(long fieldNameHash, boolean deepScan) {
        ...
        for (;;) {
            ...
            long hash;
            if (ch == '"') {
                hash = TypeUtils.fnv1a_64_magic_hashcode;
                ...
                for (int i = bp + 1; i < text.length(); ++i) {
                    ...
                    if (c == '\\') {
                        ++i;
                        if (i == text.length()) {
                            throw new com.alibaba.json.JSONException("unclosed str, " + info());
                        }
                        c = text.charAt(i);
                    }
            ...
            if (ch == ':') {
                ...
                throw new com.alibaba.json.JSONException("illegal json, " + info());
            }
