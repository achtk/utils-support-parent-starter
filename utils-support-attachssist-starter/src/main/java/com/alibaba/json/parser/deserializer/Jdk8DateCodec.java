...
import com.alibaba.json.parser.deserializer.ContextObjectDeserializer;
import com.alibaba.json.parser.deserializer.ObjectDeserializer;
import com.alibaba.json.serializer.*;
...
public class Jdk8DateCodec extends ContextObjectDeserializer implements ObjectSerializer, ContextObjectSerializer, ObjectDeserializer {
    ...
    public <T> T deserialze(DefaultJSONParser parser, Type type, Object fieldName, String format, int feature) {
        ...
        if (lexer.token() == JSONToken.LITERAL_STRING) {
                    ...
                    if (digit && text.length() > 8 && text.length() < 19) {
                        ...
                        localTime = LocalDateTime
                                .ofInstant(
                                        ...
                                        com.alibaba.json.JSON.defaultTimeZone.toZoneId())
                                .toLocalTime();
                    } else {
            ...
            if (type == LocalDateTime.class) {
                return (T) LocalDateTime.ofInstant(Instant.ofEpochMilli(millis), com.alibaba.json.JSON.defaultTimeZone.toZoneId());
            }

            if (type == LocalDate.class) {
                return (T) LocalDateTime.ofInstant(Instant.ofEpochMilli(millis), com.alibaba.json.JSON.defaultTimeZone.toZoneId()).toLocalDate();
            }
            if (type == LocalTime.class) {
                return (T) LocalDateTime.ofInstant(Instant.ofEpochMilli(millis), com.alibaba.json.JSON.defaultTimeZone.toZoneId()).toLocalTime();
            }

            if (type == ZonedDateTime.class) {
                return (T) ZonedDateTime.ofInstant(Instant.ofEpochMilli(millis), com.alibaba.json.JSON.defaultTimeZone.toZoneId());
            }
    ...
    protected LocalDateTime parseDateTime(String text, DateTimeFormatter formatter) {
        ...
        if (formatter == null) {
            ...
            if (digit && text.length() > 8 && text.length() < 19) {
                long epochMillis = Long.parseLong(text);
                return LocalDateTime.ofInstant(Instant.ofEpochMilli(epochMillis), com.alibaba.json.JSON.defaultTimeZone.toZoneId());
            }
        }
    ...
    protected ZonedDateTime parseZonedDateTime(String text, DateTimeFormatter formatter) {
        if (formatter == null) {
            ...
            if (digit && text.length() > 8 && text.length() < 19) {
                long epochMillis = Long.parseLong(text);
                return ZonedDateTime.ofInstant(Instant.ofEpochMilli(epochMillis), com.alibaba.json.JSON.defaultTimeZone.toZoneId());
            }
        }
        ...
        if (object == null) {
            ...
            if (fieldType == LocalDateTime.class) {
                ...
                if (format != null) {
                    ...
                    out.writeLong(dateTime.atZone(com.alibaba.json.JSON.defaultTimeZone.toZoneId()).toInstant().toEpochMilli());
                }
            } else {
    ...
    private void write(SerializeWriter out, TemporalAccessor object, String format) {
        ...
        if ("unixtime".equals(format)) {
            ...
            if (object instanceof LocalDateTime) {
                long seconds = ((LocalDateTime) object).atZone(com.alibaba.json.JSON.defaultTimeZone.toZoneId()).toEpochSecond();
                out.writeInt((int) seconds);
                return;
            }
        }
