...
import static com.alibaba.json.serializer.SerializerFeature.PrettyFormat;
import static com.chua.agent.support.constant.Constant.*;

/**
 * DubboProxyInterceptorHandler
 * @author CH
 */
public class DubboProxyInterceptorHandler {
    /**
     * 切入点
     * @param span 切入点
     * @param objects
    ...
    private static void registerSpan(Object value, String trim, Span lastSpan) {
        ...
        for (int i = 0, spansSize = spans.size(); i < spansSize; i++) {
            ...
            if("sql".equals(span.getModel())) {
                ServiceAgentPlugin.registerEdge("image",
                        "resources/images/dubbo.png",
                        lastSpan.getDb(),
                        "image",
                        "resources/images/"+ span.getError() +".png",
                        span.getDb());
