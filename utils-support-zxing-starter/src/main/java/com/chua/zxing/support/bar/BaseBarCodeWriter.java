...
import com.chua.common.support.lang.bar.BarCodeBorderStyle;
 ...
 * @version 1.0.0
 */
public abstract class BaseBarCodeWriter implements BarCodeWriter {
    ...
    public static final Color getColor(final String hexColor) {
        ...
        if (hexc != null && !"".equals(hexc)) {
            String hash = "#";
            if (hexc.startsWith(hash)) {
                hexc = hexc.replaceFirst(hash, "");
            }
    ...
    public static BitMatrix redrawBitMatrix(BitMatrix bitMatrix, final int padding) {
        ...
        BitMatrix matrix = new BitMatrix(rawWidth, rawHeight);
        matrix.clear();
        /**
         * redraw start
         */
    ...
    public static BufferedImage setRadius(BufferedImage image, int radius, int borderSize, String borderColor,
                                          BarCodeBorderStyle style, int borderDashGranularity, int margin) {
        ...
        if (borderSize > 0) {
            Stroke stroke = new BasicStroke(borderSize, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 10.0f,
