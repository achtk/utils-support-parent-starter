...
public class Gradient extends ArrayColormap implements Cloneable {
    ...
    public void randomize() {
        numKnots = 4 + (int) (6D * Math.random());
        xKnots = new int[numKnots];
        yKnots = new int[numKnots];
        knotTypes = new byte[numKnots];
        for (int i = 0; i < numKnots; i++) {
            xKnots[i] = (int) (255D * Math.random());
            yKnots[i] = 0xff000000 | ((int) (255D * Math.random()) << 16) | ((int) (255D * Math.random()) << 8) | (int) (255D * Math.random());
            knotTypes[i] = RGB | SPLINE;
        }
        xKnots[0] = -1;
        xKnots[1] = 0;
                ...
                if (j >= 0 && j <= 255) {
                    switch (blend) {
                        ...
                        case CIRCLE_DOWN:
                            ...
                            break;
                        default:
                            throw new IllegalStateException("Unexpected value: " + blend);
                    }
//					if (blend != SPLINE) {
                    switch (type) {
                        case RGB:
                        ...
                        case HUE_CCW:
                            ...
                            map[j] = 0xff000000 | Color.HSBtoRGB(h, s, b);
                            break;
                        default:
