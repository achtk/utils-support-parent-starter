...
public class ImageLensBlurFilter extends AbstractImageFilter {
    ...
    public BufferedImage filter(BufferedImage src, BufferedImage dst) {
        int width = src.getWidth(), height = src.getHeight(), rows = 1, cols = 1, log2rows = 0, log2cols = 0, iradius = (int) Math.ceil(radius), tileWidth = 128, tileHeight = tileWidth, adjustedWidth = width + iradius * 2, adjustedHeight = height + iradius * 2;
        ...
        for (int tileY = -iradius; tileY < height; tileY += tileHeight - TWE * iradius) {
            for (int tileX = -iradius; tileX < width; tileX += tileWidth - TWE * iradius) {
