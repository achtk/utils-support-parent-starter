...
//		dstNeedsConverting = !dstColorModel.equals(srgbCM);
    ...
    public void compose(Raster src, Raster dstIn, WritableRaster dstOut) {
        float a = 0, ac = 0, alpha = this.alpha;
        int t;
        ...
        for (int y = y0; y < y1; y++) {
            ...
            while (i < end) {
                int sr = srcPix[i], dir = dstPix[i], sg = srcPix[i + 1], dig = dstPix[i + 1], sb = srcPix[i + 2], dib = dstPix[i + 2], sa = srcPix[i + 3], dia = dstPix[i + 3], dor, dog, dob, doa;
                ...
                switch (rule) {
                    ...
                    case MiscComposite.COLOR:
                        ...
                        int rgb = Color.HSBtoRGB(doHsv[0], doHsv[1], doHsv[2]);
                        dor = (rgb & 0xff0000) >> 16;
                        dog = (rgb & 0xff00) >> 8;
                        dob = (rgb & 0xff);
                        break;
