...
public class TableRules {
    ...
    public static RelOptRule[] getRules(String tableName) {
        if (null == RULES) {
            RULES = new RelOptRule[]{
                    TableRules.getInstance(tableName, TableSortRule.class, TableSortRule::new, Sort.class, "TableSortRule"),
                    TableRules.getInstance(tableName, TableFilterRule.class, TableFilterRule::new, LogicalFilter.class, "TableLogicalFilter"),
                    TableRules.getInstance(tableName, TableProjectRule.class, TableProjectRule::new, LogicalProject.class, "TableLogicalProject"),
                    TableRules.getInstance(tableName, TableAggregateRule.class, TableAggregateRule::new, LogicalAggregate.class, "TableLogicalAggregate")
            };
        }
    ...
    private static RelOptRule getInstance(String tableName,
                                          Class<? extends ConverterRule> ruleType,
                                          Function<ConverterRule.Config, AbstractTableConverterRule> function,
                                          Class<? extends RelNode> relNodeType,
                                          String tableSortRule) {
        return ConverterRule.Config.INSTANCE.withConversion(relNodeType, Convention.NONE,
                        RelConvention.createConvention(tableName, TableRel.class),
                        tableSortRule)
                .withRuleFactory(function::apply)
                .toRule(ruleType);
    }
     ...
     * language.
     */
    static class RexToTableRulesTranslator extends RexVisitorImpl<String> {
        private final JavaTypeFactory typeFactory;
        private final List<String> inFields;

        private static final Map<SqlOperator, String> OPERATORS =
                new HashMap<>();

        static {
            // Arithmetic
            OPERATORS.put(SqlStdOperatorTable.DIVIDE, "$divide");
            OPERATORS.put(SqlStdOperatorTable.MULTIPLY, "$multiply");
            OPERATORS.put(SqlStdOperatorTable.MOD, "$mod");
            OPERATORS.put(SqlStdOperatorTable.PLUS, "$add");
            OPERATORS.put(SqlStdOperatorTable.MINUS, "$subtract");
            // Boolean
            OPERATORS.put(SqlStdOperatorTable.AND, "$and");
            OPERATORS.put(SqlStdOperatorTable.OR, "$or");
            OPERATORS.put(SqlStdOperatorTable.NOT, "$not");
            // Comparison
            OPERATORS.put(SqlStdOperatorTable.EQUALS, "$eq");
            OPERATORS.put(SqlStdOperatorTable.NOT_EQUALS, "$ne");
            OPERATORS.put(SqlStdOperatorTable.GREATER_THAN, "$gt");
            OPERATORS.put(SqlStdOperatorTable.GREATER_THAN_OR_EQUAL, "$gte");
            OPERATORS.put(SqlStdOperatorTable.LESS_THAN, "$lt");
            OPERATORS.put(SqlStdOperatorTable.LESS_THAN_OR_EQUAL, "$lte");
        }

        protected RexToTableRulesTranslator(JavaTypeFactory typeFactory,
        ...
        public String visitCall(RexCall call) {
            ...
            String stdOperator = OPERATORS.get(call.getOperator());
            if (stdOperator != null) {
            ...
            throw new IllegalArgumentException("Translation of " + call.toString()
                    + " is not supported by TableProject");
        }
     ...
     */
    abstract static class AbstractTableConverterRule extends ConverterRule {
        protected AbstractTableConverterRule(Config config) {
            super(config);
        }
    }
     ...
     * {@link RuleSort}.
     */
    private static class TableSortRule extends AbstractTableConverterRule {
        TableSortRule(Config config) {
            super(config);
        }
        ...
        public RelNode convert(RelNode rel) {
            ...
            return new RuleSort(rel.getCluster(), traitSet,
     ...
     * {@link TableFilter}.
     */
    private static class TableFilterRule extends AbstractTableConverterRule {

        TableFilterRule(Config config) {
            super(config);
        }
     ...
     * to a {@link TableProject}.
     */
    private static class TableProjectRule extends AbstractTableConverterRule {
        TableProjectRule(Config config) {
            super(config);
        }
        ...
        public RelNode convert(RelNode rel) {
            ...
            return new TableProject(project.getCluster(), traitSet,
...
*/
     ...
     * to an {@link RuleAggregate}.
     */
    private static class TableAggregateRule extends AbstractTableConverterRule {

        TableAggregateRule(Config config) {
            super(config);
        }
