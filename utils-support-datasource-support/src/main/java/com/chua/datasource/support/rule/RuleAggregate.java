...
public class RuleAggregate
    ...
    public RuleAggregate(RelOptCluster cluster, RelTraitSet traitSet,
    ...
    public Aggregate copy(RelTraitSet traitSet, RelNode input,
                          ...
                          List<AggregateCall> aggCalls) {
        try {
            return new RuleAggregate(getCluster(), traitSet, input,
    ...
    public void implement(Implementor implementor) {
        ...
        final List<String> outNames = TableRules.fieldNames(getRowType());
        int i = 0;
        if (groupSet.cardinality() == 1) {
            final String inName = inNames.get(groupSet.nth(0));
            list.add("_id: " + TableRules.maybeQuote("$" + inName));
            ++i;
        } else {
            List<String> keys = new ArrayList<>();
            for (int group : groupSet) {
                final String inName = inNames.get(group);
                keys.add(inName + ": " + TableRules.quote("$" + inName));
                ++i;
            }
        ...
        for (AggregateCall aggCall : aggCalls) {
            list.add(
                    TableRules.maybeQuote(outNames.get(i++)) + ": "
        ...
        if (groupSet.cardinality() == 1) {
            fixups = new AbstractList<String>() {
                ...
                public String get(int index) {
                    ...
                    return TableRules.maybeQuote(outName) + ": "
                            + TableRules.maybeQuote("$" + (index == 0 ? "_id" : outName));
                }
            ...
            for (int group : groupSet) {
                fixups.add(
                        TableRules.maybeQuote(outNames.get(group))
                                + ": "
                                + TableRules.maybeQuote("$_id." + outNames.get(group)));
                ++i;
            }
            for (AggregateCall ignored : aggCalls) {
                final String outName = outNames.get(i++);
                fixups.add(
                        TableRules.maybeQuote(outName) + ": " + TableRules.maybeQuote(
                                "$" + outName));
            }
        }
        if (!groupSet.isEmpty()) {
    ...
    private static String toMongo(SqlAggFunction aggregation, List<String> inNames,
        ...
        if (aggregation == SqlStdOperatorTable.COUNT) {
            ...
            assert args.size() == 1;
            final String inName = inNames.get(args.get(0));
            return "{$sum: " + TableRules.maybeQuote("$" + inName) + "}";
        } else if (aggregation == SqlStdOperatorTable.MIN) {
            assert args.size() == 1;
            final String inName = inNames.get(args.get(0));
            return "{$min: " + TableRules.maybeQuote("$" + inName) + "}";
        } else if (aggregation == SqlStdOperatorTable.MAX) {
            assert args.size() == 1;
            final String inName = inNames.get(args.get(0));
            return "{$max: " + TableRules.maybeQuote("$" + inName) + "}";
        } else if (aggregation == SqlStdOperatorTable.AVG) {
            assert args.size() == 1;
            final String inName = inNames.get(args.get(0));
            return "{$avg: " + TableRules.maybeQuote("$" + inName) + "}";
        } else {
