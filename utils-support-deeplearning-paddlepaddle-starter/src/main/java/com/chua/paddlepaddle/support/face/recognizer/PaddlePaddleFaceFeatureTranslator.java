package com.chua.paddlepaddle.support.face.recognizer;
...
/**
 * PaddlePaddleFaceFeatureTranslator
 *
 * @author CH
 */
public class PaddlePaddleFaceFeatureTranslator implements Translator<Image, float[]> {

    public PaddlePaddleFaceFeatureTranslator() {
    }
...
//    array = NDImageUtils.centerCrop(array,112,112);
        ...
        NDArray mean =
                ctx.getNDManager().create(new float[]{0.5f, 0.5f, 0.5f}, new Shape(3, 1, 1)).getNDArrayInternal().getArray();
        NDArray std =
                ctx.getNDManager().create(new float[]{0.5f, 0.5f, 0.5f}, new Shape(3, 1, 1)).getNDArrayInternal().getArray();
