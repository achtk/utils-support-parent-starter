...
public class BoundFinderV2 {
     ...
     */
    public BoundFinderV2(DetectionConfiguration configuration, boolean[][] grid) {
    ...
    public List<BoundingBox> getBoxes() {
        return pointsCollection.stream()
                ...
                .filter(rect -> {
                    return rect.getWidth() * width > MapUtils.getFloat(configuration.ext(), "limitWidth", 5.0f) && rect.getHeight() * height > MapUtils.getFloat(configuration.ext(), "limitHeight", 1.1f);
                })
