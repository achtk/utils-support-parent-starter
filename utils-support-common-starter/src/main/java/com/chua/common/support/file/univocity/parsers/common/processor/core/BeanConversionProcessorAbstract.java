...
import com.chua.common.support.file.univocity.parsers.annotations.EnumOptions;
...
import com.chua.common.support.file.univocity.parsers.annotations.helpers.*;
...
import com.chua.common.support.file.univocity.parsers.common.fields.ColumnMapper;
import com.chua.common.support.file.univocity.parsers.common.fields.ColumnMapping;
...
import static com.chua.common.support.file.univocity.parsers.annotations.helpers.AnnotationHelper.*;
 ...
 * {@link com.chua.common.support.file.univocity.parsers.annotations}.
 *
 ...
 */
public class BeanConversionProcessorAbstract<T> extends AbstractDefaultConversionProcessor {
	...
	private Object[] row;
	private Map<FieldMapping, BeanConversionProcessorAbstract<?>> nestedAttributes = null;
	protected final BaseHeaderTransformer transformer;
	 ...
	 * @deprecated Use the {@link #BeanConversionProcessorAbstract(Class, MethodFilter)} constructor instead.
	 */
	@Deprecated
	public BeanConversionProcessorAbstract(Class<T> beanType) {
	 ...
	 */
	public BeanConversionProcessorAbstract(Class<T> beanType, MethodFilter methodFilter) {
		this(beanType, null, methodFilter);
	}

	BeanConversionProcessorAbstract(Class<T> beanType, BaseHeaderTransformer transformer, MethodFilter methodFilter) {
	...
	private void processNestedField(Class nestedType, AnnotatedElement element, String targetName, PropertyWrapper propertyDescriptor, NormalizedString[] headers, Nested nested) {
		BaseHeaderTransformer transformer = null;
		if (nested != null) {
			...
			if (transformerType != BaseHeaderTransformer.class) {
				...
				transformer = AnnotationHelper.newInstance(BaseHeaderTransformer.class, transformerType, args);
			}
		}

		FieldMapping mapping = new FieldMapping(nestedType, element, propertyDescriptor, null, headers);
		BeanConversionProcessorAbstract<?> processor = createNestedProcessor(nested, nestedType, mapping, transformer);
	...
	Map<FieldMapping, BeanConversionProcessorAbstract<?>> getNestedAttributes() {
		if (nestedAttributes == null) {
			nestedAttributes = new LinkedHashMap<FieldMapping, BeanConversionProcessorAbstract<?>>();
		}
		return nestedAttributes;
	}

	BeanConversionProcessorAbstract<?> createNestedProcessor(Annotation annotation, Class nestedType, FieldMapping fieldMapping, BaseHeaderTransformer transformer) {
		return new BeanConversionProcessorAbstract<Object>(nestedType, transformer, methodFilter);
	}
	 ...
	 *                         been defined in {@link BaseCommonSettings#getHeaders()}
	 ...
	 *                         {@link BaseCommonSettings#selectFields(String...)} or {@link BaseCommonSettings#selectIndexes(Integer...)}
	...
	private void initializeValuesForMissing() {
		if (nonNullReadOrderLength() < parsedFields.size()) {
			...
			BeanConversionProcessorAbstract tmp = new BeanConversionProcessorAbstract(getBeanClass(), methodFilter) {
				protected void addConversion(Conversion conversion, FieldMapping mapping) {
					if (conversion == null) {
						return;
					}
	...
	void processNestedAttributes(String[] row, Object instance, AbstractContext context) {
		for (Map.Entry<FieldMapping, BeanConversionProcessorAbstract<?>> e : nestedAttributes.entrySet()) {
	 ...
	 *                       {@link BaseCommonSettings#getHeaders()}
	...
	public final Object[] reverseConversions(T bean, NormalizedString[] headers, int[] indexesToWrite) {
		...
		if (nestedAttributes != null) {
			for (Map.Entry<FieldMapping, BeanConversionProcessorAbstract<?>> e : nestedAttributes.entrySet()) {
				...
				if (nested != null) {
					BeanConversionProcessorAbstract<Object> nestedProcessor = (BeanConversionProcessorAbstract<Object>) e.getValue();
	...
	private void populateTargetMaps(Map<Object, Integer> targetCounts, Map<Object, String> targetSources) {
		...
		if (nestedAttributes != null) {
			for (BeanConversionProcessorAbstract nestedProcessor : nestedAttributes.values()) {
