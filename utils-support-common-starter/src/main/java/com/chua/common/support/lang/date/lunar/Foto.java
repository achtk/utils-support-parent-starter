...
public class Foto {
    ...
    public Foto(LunarTime lunar) {
        this.lunar = lunar;
    }

    public static Foto fromLunar(LunarTime lunar) {
        return new Foto(lunar);
    }

    public static Foto fromYmdHms(int year, int month, int day, int hour, int minute, int second) {
        return Foto.fromLunar(LunarTime.fromYmdHms(year + DEAD_YEAR - 1, month, day, hour, minute, second));
    }
    ...
    public boolean isDayZhaiSix() {
        int d = getDay();
        int dy = 28;
        boolean b = 8 == d || 14 == d || 15 == d || 23 == d || 29 == d || 30 == d;
        if (b) {
            return true;
        } else if (dy == d) {
