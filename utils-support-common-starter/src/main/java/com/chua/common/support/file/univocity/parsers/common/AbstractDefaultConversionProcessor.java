 ...
 */
public abstract class AbstractDefaultConversionProcessor implements ConversionProcessor {
	...
	/**
	 * Executes the sequences of reverse conversions defined using {@link AbstractDefaultConversionProcessor#convertFields(Conversion...)}, {@link AbstractDefaultConversionProcessor#convertIndexes(Conversion...)} and {@link AbstractDefaultConversionProcessor#convertAll(Conversion...)}, for every field in the given row.
	 *
	 ...
	 * {@link BaseRetryableErrorHandler}, the user can provide a default value to use in place of the one that could not
	 * be converted, and decide whether or not the record should be kept with the use of the
	 * {@link BaseRetryableErrorHandler#keepRecord()} method.
	 *
	...
	protected final boolean handleConversionError(Throwable ex, Object[] row, int column) {
		...
		if (column > -1 && errorHandler instanceof BaseRetryableErrorHandler) {
			((BaseRetryableErrorHandler) errorHandler).prepareToRun();
		}
	...
	public final void convertType(Class<?> type, Conversion... conversions) {
		...
		if (conversionsByType == null) {
			conversionsByType = new LinkedHashMap<>();
		}
