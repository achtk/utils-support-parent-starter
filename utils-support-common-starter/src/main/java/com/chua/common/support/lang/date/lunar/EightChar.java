...
public class EightChar {
     ...
     * 阴历
     */
    protected LunarTime lunar;

    public EightChar(LunarTime lunar) {
        this.lunar = lunar;
    }

    public static EightChar fromLunar(LunarTime lunar) {
    ...
    public String getTaiYuan() {
        ...
        int ganIndexCount = 10;
        if (ganIndex >= ganIndexCount) {
            ganIndex -= 10;
        }
        int zhiIndex = lunar.getMonthZhiIndexExact() + 3;
        int zhiIndexCount = 12;
        if (zhiIndex >= zhiIndexCount) {
            zhiIndex -= 12;
        }
    ...
    public String getMingGong() {
        ...
        int jiaZiIndexCount = 60;
        if (jiaZiIndex >= jiaZiIndexCount) {
            jiaZiIndex -= 60;
        }
        if (jiaZiIndex < 0) {
            jiaZiIndex += 60;
