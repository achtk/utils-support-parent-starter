...
public class ArgumentUtils {
	...
	public static Reader newReader(InputStream input, Charset encoding) {
		if (encoding == null) {
            ...
            //charset detected. Just set the encoding and keep using the original input stream.
            if (bomInput.getEncoding() != null) {
                encoding = bomInput.getCharset();
            }

            //there are bytes to be processed. We should use the BomInput wrapper to read the first bytes already consumed when trying to match the BOM.
	...
	public static String trim(String input, boolean left, boolean right) {
		boolean express = input.length() == 0 || !left && !right;
		if (express) {
			return input;
		}
		int begin = 0;
	...
	public static String displayLineSeparators(String str, boolean addNewLine) {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			if (ch == '\r' || ch == '\n') {
				...
				if (i + 1 < str.length()) {
					next = str.charAt(i + 1);
					boolean exp = next != ch && (next == '\r' || next == '\n');
					if (exp) {
