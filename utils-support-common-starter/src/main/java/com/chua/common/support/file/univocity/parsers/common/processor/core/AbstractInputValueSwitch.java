...
import com.chua.common.support.file.univocity.parsers.common.processor.CustomMatcher;
...
public abstract class AbstractInputValueSwitch<T extends Context> extends AbstractProcessorSwitch<T> {
    ...
    private static final Comparator<String> CASE_SENSITIVE_COMPARATOR = (o1, o2) -> Objects.equals(o1, o2) ? 0 : 1;

    private static final Comparator<String> CASE_INSENSITIVE_COMPARATOR = (o1, o2) -> {
        boolean b = Objects.equals(o1, o2) || o1 != null && o1.equalsIgnoreCase(o2);
        return b ? 0 : 1;
    };

    private Comparator<String> comparator = CASE_INSENSITIVE_COMPARATOR;
	...
	protected final Processor<T> switchRowProcessor(String[] row, T context) {
		...
		if (columnIndex < row.length) {
			...
			for (int i = 0; i < switches.length; i++) {
				Switch s = switches[i];
				boolean express = (s.matcher != null && s.matcher.matches(valueToMatch)) || comparator.compare(valueToMatch, s.value) == 0;
				if (express) {
					headers = s.headers;
					indexes = s.indexes;
					return s.processor;
