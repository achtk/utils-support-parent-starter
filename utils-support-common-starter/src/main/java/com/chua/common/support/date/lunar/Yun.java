...
public class Yun {
    ...
    private void computeStart() {
        ...
        // 出生日期
        SolarTime current = lunar.getSolar();
        // 阳男阴女顺推，阴男阳女逆推
        SolarTime start = forward ? current : prev.getSolar();
        SolarTime end = forward ? next.getSolar() : current;
        ...
        Calendar endCalendar = DateUtils.toCalendar(end.getYear(), end.getMonth(), end.getDay());
     ...
     */
    @SuppressWarnings("MagicConstant")
    public SolarTime getStartSolar() {
        SolarTime birth = lunar.getSolar();
        Calendar c = DateUtils.toCalendar(birth.getYear(), birth.getMonth(), birth.getDay());
        c.add(Calendar.YEAR, startYear);
        c.add(Calendar.MONTH, startMonth);
        c.add(Calendar.DATE, startDay);
        return SolarTime.fromCalendar(c);
    }
