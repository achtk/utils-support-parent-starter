 ...
 * {@link com.chua.common.support.jsoup.parser.Parser#setTrackPosition(boolean)}.
 *
...
public class Range {
    ...
    private static final String RANGE_KEY = Attributes.internalKey("jsoup.sourceRange");
    private static final String END_RANGE_KEY = Attributes.internalKey("jsoup.endSourceRange");
    private static final Position UNTRACKED_POS = new Position(-1, -1, -1);
    private static final Range UNTRACKED = new Range(UNTRACKED_POS, UNTRACKED_POS);
    ...
    static Range of(Node node, boolean start) {
        final String key = start ? RANGE_KEY : END_RANGE_KEY;
        if (!node.hasAttr(key)) {
            return UNTRACKED;
        } else {
    ...
    public void track(Node node, boolean start) {
        node.attributes().putUserData(start ? RANGE_KEY : END_RANGE_KEY, this);
    }
