...
public class FileUtils {
    ...
    private static final String[] ZIP_COMPRESS = new String[]{"zip", "jar"};
    private static final String[] COMPRESS =
            new String[]{"zip", "jar", "tar", "xz", "tar.xz", "zip.xz", "zip.gz", "tar.gz"};
     ...
     * @return 文件
     */
    public static File createTempFile(String path, String sourceFileName) {
        File tmp = new File(path);
        mkdir(tmp);
        try {
            return Files.createTempFile(tmp.toPath(), DateTime.now().toFormat(DateFormatConstant.YYYYMMDD) + "_", "." + FileUtils.getExtension(sourceFileName)).toFile();
        } catch (IOException e) {
     ...
     *
     * <p>This method delegates to {@link Files#isSymbolicLink(Path path)}
     *
    ...
    /**
     * 删除后缀 foo.txt --&gt; foo a\b\c.jpg --&gt; a\b\c a\b\c --&gt; a\b\c a.b\c --&gt; a.b\c </pre>
     *
     * @param filename the filename to query, null returns null
    ...
    /**
     * 获取全路径 C:\a\b\c.txt --&gt; C:\a\b\ ~/a/b/c.txt --&gt; ~/a/b/ a.txt --&gt; "" a/b/c --&gt; a/b/
     * a/b/c/ --&gt; a/b/c/ C: --&gt; C: C:\ --&gt; C:\ ~ --&gt; ~/ ~/ --&gt; ~/ ~user --&gt; ~user/
     * ~user/ --&gt; ~user/ </pre>
     *
     * @param filename the filename to query, null returns null
    ...
    public static boolean wildcardMatches(final String filename, final String wildcardMatcher) {
        List<String> strings =
                Splitter.on(PATTERN_EMPTY).trimResults().omitEmptyStrings().splitToList(wildcardMatcher);
    ...
    public static String toFolder(final String folder) {
        return StringUtils.isEmpty(folder)
                ? SYMBOL_EMPTY
                : (folder.endsWith(SYMBOL_LEFT_SLASH) ? folder : folder + SYMBOL_LEFT_SLASH);
    }
    ...
    public static File tryFile(URL url) {
        ...
        try {
            ...
            if (path.contains(JAR_FILE_EXTENSION_IN)) {
                path =
                        path.substring(
                                0, path.lastIndexOf(JAR_FILE_EXTENSION_IN) + JAR_FILE_EXTENSION.length());
            }
            if ((file = new File(path)).exists()) {
                return file;
            }
    ...
    public static List<File> ofVolumes() {
        ...
        return ofVolumesFile().stream()
                .map(file -> new File(fileSystemView.getSystemDisplayName(file)))
    ...
    public static void copyFiles(URL[] urls, String targetPath, boolean overlay) throws IOException {
        ...
        copyFiles(
                Arrays.stream(urls).map(url -> new File(url.getFile())).toArray(File[]::new),
                targetPath,
                overlay);
    }
     ...
     * @throws IOException IOException
     */
    public static void copyFiles(File[] files, String targetPath, boolean overlay)
            throws IOException {
        createIfNotExist(targetPath);
     ...
     * @throws IOException IOException
     */
    public static void copyFile(File sourceFile, File targetPath, boolean overlay)
            throws IOException {
     ...
     * @throws IOException IOException
     */
    public static void copyFile(File sourceFile, String targetPath, boolean overlay)
            throws IOException {
        if (null == sourceFile) {
            return;
        }
     ...
     *
     * <ul>
     *   <li>当文件为空，从开头追加，尾部不加空行
     *   <li>当有内容，换行追加，尾部不加空行
     *   <li>当有内容，并末尾有空行，依旧换行追加
     * </ul>
     *
     * @param <T>     集合元素类型
     * @param list    列表
    ...
    public static <T> File writeLines(
            Collection<T> list, String path, String charset, boolean isAppend) throws IOException {
     ...
     */
    public static <T> File writeLines(
            Collection<T> list, String path, Charset charset, boolean isAppend) throws IOException {
        return writeLines(list, file(path), charset, isAppend);
    }
    ...
    public static <T> File writeLines(Collection<T> list, File file, String charset, boolean isAppend)
            throws IOException {
        return com.chua.common.support.file.FileWriter.create(file, Charset.forName(charset))
                .writeLines(list, isAppend);
    }
     ...
     */
    public static <T> File writeLines(
            Collection<T> list, File file, Charset charset, boolean isAppend) throws IOException {
    ...
    public static File writeUtf8Map(Map<?, ?> map, File file, String kvSeparator, boolean isAppend)
            throws IOException {
        return com.chua.common.support.file.FileWriter.create(file, UTF_8)
    ...
    public static File writeMap(
            ...
            throws IOException {
        return com.chua.common.support.file.FileWriter.create(file, charset)
                .writeMap(map, kvSeparator, isAppend);
    }
     ...
     * @return 结果
     */
    public static Collection<String> find(String path, String name) {
        ...
        if (urls.isEmpty()) {
            for (String s : split) {
                urls.addAll(doAnalysis(s, name, false));
            }
        }

        if (urls.isEmpty()) {
            doAnalysisUserHome(urls, split, false);
        }

        return urls;
    }

    public static List<String> doAnalysis(String path, String name, boolean isDirector) {
        boolean isLike = name.contains("*");
        List<String> urls = new LinkedList<>();
        log.info("检测当前目录是否存在模型 : {}", new File(path).getAbsolutePath());
        try {
            Files.walkFileTree(Paths.get(path), new SimpleFileVisitor<Path>() {
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                    ...
                    String fileName = file.toFile().getName().replace("\\", "/");
                    if (isLike && PathMatcher.INSTANCE.match(name, fileName)) {
                        urls.add(file.toFile().getAbsolutePath());
                ...
                public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) throws IOException {
                    ...
                    String fileName = dir.toFile().getName().replace("\\", "/");
                    if (isLike && PathMatcher.INSTANCE.match(name, fileName)) {
                        urls.add(dir.toFile().getAbsolutePath());
                    } else if (name.equals(fileName)) {
                        urls.add(dir.toFile().getAbsolutePath());
                    }
    ...
    public static void doAnalysisUserHome(List<String> urls, String[] url, boolean isDirector) {
