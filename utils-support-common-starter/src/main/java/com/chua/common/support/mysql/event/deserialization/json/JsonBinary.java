 ...
 */
package com.chua.common.support.mysql.event.deserialization.json;

import com.chua.common.support.mysql.event.deserialization.AbstractRowsEventDataDeserializer;
import com.chua.common.support.mysql.event.deserialization.ColumnType;
 ...
 *   doc ::= type value
 *   type ::=
 *       0x00 |   *       0x01 |   *       0x02 |   *       0x03 |   *       0x04 |   *       0x05 |   *       0x06 |   *       0x07 |   *       0x08 |   *       0x09 |   *       0x0a |   *       0x0b |   *       0x0c |   *       0x0f     *   value ::=
 *       object  |
 *       array   |
 *       literal |
 *       number  |
 ...
 *    *   element-count ::=
 *       uint16 |   *       uint32     *    *   size ::=
 *       uint16 |   *       uint32     *   key-entry ::= key-offset key-length
 *   key-offset ::=
 *       uint16 |   *       uint32     *   key-length ::= uint16     *   value-entry ::= type offset-or-inlined-value
 *    *    *    *   offset-or-inlined-value ::=
 *       uint16 |    *       uint32      *   key ::= utf8mb4-data
 *   literal ::=
 *       0x00 |    *       0x01 |    *       0x02 |    *   number ::=  ....   *                      *                      *   string ::= data-length utf8mb4-data
 *   custom-data ::= custom-type data-length binary-data
 *   custom-type ::= uint8    *                            *   data-length ::= uint8*   *                            *                            *                            *                            *                           // lengths up to 16383, and so on...
 * </pre>
 *
...
public class JsonBinary {
     ...
     *        *   element-count ::=
     *       uint16 |       *       uint32         *        *   size ::=
     *       uint16 |       *       uint32         *   value-entry ::= type offset-or-inlined-value
     *        *        *        *   offset-or-inlined-value ::=
     *       uint16 |        *       uint32          *   key ::= utf8mb4-data
     *   literal ::=
     *       0x00 |        *       0x01 |        *       0x02 |        *   number ::=  ....       *                          *                          *   string ::= data-length utf8mb4-data
    ...
    protected void parseOpaque(JsonFormatter formatter) throws IOException {
        ...
        switch (type) {
            ...
            case NEWDECIMAL:
                ...
                break;                        // https://github.com/mysql/mysql-server/blob/5.7/sql/json_dom.cc#L1681
    ...
    protected void parseTime(JsonFormatter formatter) throws IOException {
        ...
        int min = (int) (value >> 6) % (1 << 6);
        int sec = (int) value % (1 << 6);
        if (negative) {
            hour *= -1;
        }
