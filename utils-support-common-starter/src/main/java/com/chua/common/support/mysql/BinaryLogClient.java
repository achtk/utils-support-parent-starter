...
import com.chua.common.support.mysql.event.deserialization.*;
import com.chua.common.support.mysql.event.deserialization.EventDeserializer.EventDataWrapper;
import com.chua.common.support.mysql.io.ByteArrayInputStream;
import com.chua.common.support.mysql.jmx.BinaryLogClientMXBean;
import com.chua.common.support.mysql.network.*;
import com.chua.common.support.mysql.network.protocol.*;
import com.chua.common.support.mysql.network.protocol.command.*;
...
public class BinaryLogClient implements BinaryLogClientMXBean {
    ...
    private PacketChannel openChannelToBinaryLogStream(final long connectTimeout) throws IOException {
        ...
        if (log.isTraceEnabled()) {
            ...
            log.trace("Connected to " + hostname + ":" + port + " at " + position +
    ...
    private ExecutorService spawnKeepAliveThread(final long connectTimeout) {
        final ExecutorService threadExecutor = ThreadUtils.newSingleThreadExecutor(new ThreadFactory() {
        ...
        threadExecutor.submit(new Runnable() {
            ...
            public void run() {
                ...
                while (!threadExecutor.isShutdown()) {
                    ...
                    if (connectionLost) {
                        if (log.isTraceEnabled()) {
                            log.trace("Trying to restore lost connection to " + hostname + ":" + port);
                        }
                        try {
                            terminateConnect();
                            ...
                            if (log.isWarnEnabled()) {
                                log.warn("Failed to restore connection to " + hostname + ":" + port +
