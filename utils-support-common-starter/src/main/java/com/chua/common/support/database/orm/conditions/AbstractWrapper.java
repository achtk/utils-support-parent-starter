...
import static com.chua.common.support.database.orm.enums.SqlKeyword.*;
import static com.chua.common.support.database.orm.enums.WrapperKeyword.APPLY;
...
public abstract class AbstractWrapper<T, R, Children extends AbstractWrapper<T, R, Children>> extends Wrapper<T>
    ...
    public Children last(boolean condition, String lastSql) {
        if (condition) {
            this.lastSql.setStringValue(CommonConstant.SYMBOL_SPACE + lastSql);
        }
        return typedThis;
    }
    ...
    public Children groupBy(boolean condition, R column, R... columns) {
        return maybeDo(condition, () -> {
            ...
            if (ArrayUtils.isNotEmpty(columns)) {
                one += (CommonConstant.SYMBOL_COMMA + columnsToString(columns));
            }
    ...
    protected final String formatSqlMaybeWithParam(String sqlStr, String mapping, Object... params) {
        ...
        if (ArrayUtils.isNotEmpty(params)) {
            for (int i = 0; i < params.length; ++i) {
                final String target = SYMBOL_LEFT_BRACE + i + SYMBOL_RIGHT_BRACE;
    ...
    protected ISqlSegment inExpression(Collection<?> value) {
        ...
        return () -> value.stream().map(i -> formatParam(null, i))
            .collect(joining(SYMBOL_COMMA, SYMBOL_LEFT_BRACKET, SYMBOL_RIGHT_BRACKET));
    }
    ...
    public String getParamAlias() {
        return paramAlias == null ? SYMBOL_WRAPPER : paramAlias.getStringValue();
    }
    ...
    public Children setParamAlias(String paramAlias) {
        Preconditions.notEmpty(paramAlias, "paramAlias can not be empty!");
        Preconditions.isEmpty(paramNameValuePairs, "Please call this method before working!");
        Preconditions.isNull(this.paramAlias, "Please do not call the method repeatedly!");
    ...
    protected String columnsToString(R... columns) {
        return Arrays.stream(columns).map(this::columnToString).collect(joining(SYMBOL_COMMA));
    }
    ...
    protected String columnsToString(List<R> columns) {
        return columns.stream().map(this::columnToString).collect(joining(SYMBOL_COMMA));
    }
