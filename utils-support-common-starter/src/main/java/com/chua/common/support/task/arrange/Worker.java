 ...
 */
public class Worker<T, V> extends TreeViewNode {
    /**
     * 该wrapper的唯一标识
     */
    private String id;
     ...
     */
    private T param;
    private WorkerAction<T, V> worker;
     ...
     * 如1后面有2、3
     */
    private List<Worker<?, ?>> nextWrappers;
    /**
     ...
     */
    private Map<String, Worker> forParamUseWrappers;
    /**
     * 也是个钩子变量，用来存临时的结果
     */
    ...
    private Worker(String id, WorkerAction<T, V> worker, T param, Callback<T, V> callback) {
        super(id);
        if (worker == null) {
     ...
     */
    private void work(ExecutorService executorService, Worker fromWrapper, long remainTime, Map<String, Worker> forParamUseWrappers) {
    ...
    public void work(ExecutorService executorService, long remainTime, Map<String, Worker> forParamUseWrappers) {
    ...
    private boolean checkNextWrapperResult() {
        ...
        Worker nextWrapper = nextWrappers.get(0);
    ...
    private void beginNext(ExecutorService executorService, long now, long remainTime) {
        ...
        if (nextWrappers.size() == 1) {
            nextWrappers.get(0).work(executorService, Worker.this, remainTime - costTime, forParamUseWrappers);
            return;
        }
        ...
        for (int i = 0; i < nextWrappers.size(); i++) {
            ...
            futures[i] = CompletableFuture.runAsync(() -> nextWrappers.get(finalI)
                    .work(executorService, Worker.this, remainTime - costTime, forParamUseWrappers), executorService);
        }
        try {
    ...
    private void doDependsOneJob(Worker dependWrapper) {
    ...
    private synchronized void doDependsJobs(ExecutorService executorService, List<DependWrapper> dependWrappers, Worker fromWrapper, long now, long remainTime) {
        ...
        for (DependWrapper dependWrapper : mustWrapper) {
            Worker worker = dependWrapper.getDependWrapper();
            WorkResult tempWorkResult = worker.getWorkResult();
            //为null或者isWorking，说明它依赖的某个任务还没执行到或没执行完
            if (worker.getState() == INIT || worker.getState() == WORKING) {
                existNoFinish = true;
                break;
            }
            ...
            if (ResultState.EXCEPTION == tempWorkResult.getResultState()) {
                workResult = defaultExResult(worker.getWorkResult().getEx());
                hasError = true;
                break;
            }
    ...
    public List<Worker<?, ?>> getNextWorker() {
        return nextWrappers;
    }
    ...
    private void addDepend(Worker<?, ?> worker, boolean must) {
        addDepend(new DependWrapper(worker, must));
    }
    ...
    private void addNext(Worker<?, ?> worker) {
        if (nextWrappers == null) {
            nextWrappers = new ArrayList<>();
        }
        //避免添加重复
        for (Worker wrapper : nextWrappers) {
            if (worker.equals(wrapper)) {
                return;
            }
        }
        addChildren(worker);
        nextWrappers.add(worker);
    }

    private void addNextWrappers(List<Worker<?, ?>> wrappers) {
        if (wrappers == null) {
            return;
        }
        for (Worker<?, ?> wrapper : wrappers) {
            addNext(wrapper);
        }
    }
     ...
     * @param worker worker
     */
    public void setNextWorker(List<Worker> worker) {
        for (Worker worker1 : worker) {
            setNextWorker(worker1);
        }
    }
    ...
    public static class Builder<W, C> {
         ...
         */
        private W param;
        private WorkerAction<W, C> worker;
        private Callback<W, C> callback;
        /**
         * 自己后面的所有
         */
        private List<Worker<?, ?>> nextWrappers;
         ...
         * 存储强依赖于自己的wrapper集合
         */
        private Set<Worker<?, ?>> selfIsMustSet;

        private boolean needCheckNextWrapperResult = true;

        public Builder<W, C> worker(WorkerAction<W, C> worker) {
            this.worker = worker;
            return this;
        }
        ...
        public Builder<W, C> callback(Callback<W, C> callback) {
            this.callback = callback;
            return this;
        }

        public Builder<W, C> depend(Worker<?, ?>... wrappers) {
            if (wrappers == null) {
                return this;
            }
            for (Worker<?, ?> wrapper : wrappers) {
        ...
        public Builder<W, C> depend(Worker<?, ?> wrapper) {
            return depend(wrapper, true);
        }

        public Builder<W, C> depend(Worker<?, ?> wrapper, boolean isMust) {
            if (wrapper == null) {
                return this;
            }
        ...
        public Builder<W, C> next(Worker<?, ?> wrapper) {
            return next(wrapper, true);
        }

        public Builder<W, C> next(Worker<?, ?> wrapper, boolean selfIsMust) {
            if (nextWrappers == null) {
                nextWrappers = new ArrayList<>();
            }
        ...
        public Builder<W, C> next(Worker<?, ?>... wrappers) {
            if (wrappers == null) {
                return this;
            }
            for (Worker<?, ?> wrapper : wrappers) {
        ...
        public Worker<W, C> build() {
            Worker<W, C> wrapper = new Worker<>(id, worker, param, callback);
            ...
            if (nextWrappers != null) {
                for (Worker<?, ?> worker : nextWrappers) {
                    boolean must = false;
                    if (selfIsMustSet != null && selfIsMustSet.contains(worker)) {
                        must = true;
                    }
                    worker.addDepend(wrapper, must);
                    wrapper.addNext(worker);
                }
            }
