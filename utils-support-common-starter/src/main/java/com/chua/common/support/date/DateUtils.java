...
public class DateUtils {
    ...
    public static String format(TemporalAccessor date) {
        if (date instanceof LocalDateTime) {
            return format((LocalDateTime) date, YYYY_MM_DD_HH_MM_SS);
        }

        if (date instanceof LocalDate) {
            return format((LocalDate) date, YYYY_MM_DD);
        }

        if (date instanceof LocalTime) {
            return format((LocalTime) date, HH_MM_SS);
        }
    ...
    public static LocalDateTime toLocalDateTime(final String date, final String pattern) {
        try {
            return toLocalDateTime(parseDate(date, Optional.ofNullable(pattern).orElse(YYYY_MM_DD_HH_MM_SS)));
