 ...
 * @since 2021-12-29
 */
public class LunarTime implements LunarDateTime {
    /**
     * 1天对应的毫秒
     */
     ...
     * 24节气表（对应阳历的准确时刻）
     */
    private final Map<String, SolarTime> jieQi = new LinkedHashMap<>();
     ...
     */
    public LunarTime(int lunarYear, int lunarMonth, int lunarDay) {
    ...
    public LunarTime(int lunarYear, int lunarMonth, int lunarDay, int hour, int minute, int second) {
        ...
        this.minute = minute;
        this.second = second;
        SolarTime noon = SolarTime.fromJulianDay(m.getFirstJulianDay() + lunarDay - 1);
        this.solar = SolarTime.fromYmdHms(noon.getYear(), noon.getMonth(), noon.getDay(), hour, minute, second);
        compute(y);
    }
     ...
     * @param dateTime 阳历日期
     */
    public LunarTime(DateTime dateTime) {
     ...
     * @param date 阳历日期
     */
    public LunarTime(Date date) {
        solar = new SolarTime(date);
        Calendar c = DateUtils.toCalendar(solar.getYear(), solar.getMonth(), solar.getDay());
        long solarTime = c.getTimeInMillis();
        int y = solar.getYear();
        LunarYear ly = LunarYear.fromYear(y);
        for (LunarMonth m : ly.getMonths()) {
            // 初一
            Calendar firstDay = SolarTime.fromJulianDay(m.getFirstJulianDay()).getCalendar();
    ...
    private void computeJieQi(LunarYear lunarYear) {
        ...
        for (int i = 0, j = JIE_QI_IN_USE.length; i < j; i++) {
            jieQi.put(JIE_QI_IN_USE[i], SolarTime.fromJulianDay(julianDays.get(i)));
        }
    }
    ...
    private void computeDay() {
        SolarTime noon = SolarTime.fromYmdHms(solar.getYear(), solar.getMonth(), solar.getDay(), 12, 0, 0);
        ...
        String n = "23:00";
        String n1 = "23:59";
        int step10 = 10, step12 = 12;
        if (hm.compareTo(n) >= 0 && hm.compareTo(n1) <= 0) {
            dayGanExact++;
            if (dayGanExact >= step10) {
                dayGanExact -= 10;
            }
            dayZhiExact++;
            if (dayZhiExact >= step12) {
                dayZhiExact -= 12;
            }
        }
     ...
     * @return 农历
     */
    public static LunarTime fromDate(Date date) {
        return new LunarTime(date);
    }
     ...
     * @return 农历
     */
    public static LunarTime fromYmd(int lunarYear, int lunarMonth, int lunarDay) {
        return new LunarTime(lunarYear, lunarMonth, lunarDay);
    }
    ...
    protected String convertJieQi(String name) {
        String jq = name;
        String dz = "DONG_ZHI", dh = "DA_HAN", xh = "XIAO_HAN", lc = "LI_CHUN", dx = "DA_XUE", ys = "YU_SHUI", jz = "JING_ZHE";
        if (dz.equals(jq)) {
            jq = "冬至";
        } else if (dh.equals(jq)) {
            jq = "大寒";
        } else if (xh.equals(jq)) {
            jq = "小寒";
        } else if (lc.equals(jq)) {
            jq = "立春";
        } else if (dx.equals(jq)) {
            jq = "大雪";
        } else if (ys.equals(jq)) {
            jq = "雨水";
        } else if (jz.equals(jq)) {
            jq = "惊蛰";
        }
        return jq;
    }
    ...
    public String getJie() {
        String jie = "";
        int step2 = 2;
        for (int i = 0, j = JIE_QI_IN_USE.length; i < j; i += step2) {
            String key = JIE_QI_IN_USE[i];
            SolarTime d = jieQi.get(key);
    ...
    public String getQi() {
        String qi = "";
        int step2 = 2;
        for (int i = 1, j = JIE_QI_IN_USE.length; i < j; i += step2) {
            String key = JIE_QI_IN_USE[i];
            SolarTime d = jieQi.get(key);
            if (d.getYear() == solar.getYear() && d.getMonth() == solar.getMonth() && d.getDay() == solar.getDay()) {
                qi = key;
    ...
    public List<String> getFestivals() {
        ...
        int step12 = 12;
        int step29 = 29;
        if (Math.abs(month) == step12 && day >= step29 && year != next(1).getYear()) {
            l.add("除夕");
        }
        return l;
    }
    ...
    public List<String> getOtherFestivals() {
        ...
        String qm = "清明";
        if (solar.toYmd().equals(jieQi.get(qm).next(-1).toYmd())) {
            l.add("寒食节");
        }
        return l;
    }
    ...
    public NineStar getMonthNineStar() {
        ...
        String zw = "子午卯酉";
        String c = "辰戌丑未";
        if (zw.contains(yearZhi)) {
            start = 8;
        } else if (c.contains(yearZhi)) {
            start = 5;
        }
        // 寅月起，所以需要-2
    ...
    public NineStar getTimeNineStar() {
        ...
        String dz = "冬至";
        String xz = "夏至";
        if (solarYmd.compareTo(jieQi.get(dz).toYmd()) >= 0 && solarYmd.compareTo(jieQi.get(xz).toYmd()) < 0) {
            asc = true;
        }
        int start = asc ? 7 : 3;
        ...
        String z = "子午卯酉";
        String c = "辰戌丑未";
        if (z.contains(dayZhi)) {
            start = asc ? 1 : 9;
        } else if (c.contains(dayZhi)) {
            start = asc ? 4 : 6;
        }
     ...
     * @return 节气表
     */
    public Map<String, SolarTime> getJieQiTable() {
        return jieQi;
    }
    ...
    protected JieQi getNearJieQi(boolean forward, String[] conditions, boolean wholeDay) {
        ...
        for (Map.Entry<String, SolarTime> entry : jieQi.entrySet()) {
            ...
            SolarTime solar = entry.getValue();
    ...
    public String getJieQi() {
        String name = "";
        for (Map.Entry<String, SolarTime> jq : jieQi.entrySet()) {
            SolarTime d = jq.getValue();
            if (d.getYear() == solar.getYear() && d.getMonth() == solar.getMonth() && d.getDay() == solar.getDay()) {
                name = jq.getKey();
                break;
            }
    ...
    public String toFullString() {
        ...
        s.append(getYearInGanZhi()).append("(");
        s.append(getYearShengXiao()).append(")年 ");
        s.append(getMonthInGanZhi()).append("(");
        s.append(getMonthShengXiao()).append(")月 ");
        s.append(getDayInGanZhi()).append("(");
        s.append(getDayShengXiao()).append(")日 ");
        s.append(getTimeZhi());
        s.append("(");
     ...
     * @return 农历日期
     */
    public LunarTime next(int days) {
    ...
    public ShuJiu getShuJiu() {
        Calendar currentCalendar = DateUtils.toCalendar(solar.getYear(), solar.getMonth(), solar.getDay());
        SolarTime start = jieQi.get("DONG_ZHI");
        Calendar startCalendar = DateUtils.toCalendar(start.getYear(), start.getMonth(), start.getDay());

        if (currentCalendar.compareTo(startCalendar) < 0) {
            start = jieQi.get("冬至");
            startCalendar = DateUtils.toCalendar(start.getYear(), start.getMonth(), start.getDay());
        }

        Calendar endCalendar = DateUtils.toCalendar(start.getYear(), start.getMonth(), start.getDay());
    ...
    public Fu getFu() {
        Calendar currentCalendar = DateUtils.toCalendar(solar.getYear(), solar.getMonth(), solar.getDay());
        SolarTime xiaZhi = jieQi.get("夏至");
        SolarTime liQiu = jieQi.get("立秋");
        Calendar startCalendar = DateUtils.toCalendar(xiaZhi.getYear(), xiaZhi.getMonth(), xiaZhi.getDay());
        // 第1个庚日
        ...
        Calendar liQiuCalendar = DateUtils.toCalendar(liQiu.getYear(), liQiu.getMonth(), liQiu.getDay());
    ...
    public String getWuHou() {
        ...
        SolarTime startSolar = jieQi.getSolar();
        Calendar startCalendar = DateUtils.toCalendar(startSolar.getYear(), startSolar.getMonth(), startSolar.getDay());
     ...
     * @return 节气
     */
    public Map<String, SolarTime> getJieQiMapping() {
        return jieQi;
    }
}
