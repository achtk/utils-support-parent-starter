...
/**
 * This is the parent class for all configuration classes used by parsers ({@link BaseParser}) and writers ({@link AbstractWriter})
 *
...
public abstract class BaseCommonSettings<F extends BaseFormat> implements Cloneable {
	 ...
	 * set {@link BaseCommonParserSettings#setColumnReorderingEnabled(boolean)} with {@code false}.</p>
	 *
	...
	/**
	 * Returns the custom error handler to be used to capture and handle errors that might happen while processing records with a {@link com.chua.common.support.file.univocity.parsers.common.processor.core.Processor}
	 * or a {@link RowWriterProcessor} (i.e. non-fatal {@link DataProcessingException}s).
	 *
	 * <p>The parsing/writing process won't stop (unless the error handler rethrows the {@link DataProcessingException} or manually stops the process).</p>
	 *
	...
	/**
	 * Defines a custom error handler to capture and handle errors that might happen while processing records with a {@link com.chua.common.support.file.univocity.parsers.common.processor.core.Processor}
	 * or a {@link RowWriterProcessor} (i.e. non-fatal {@link DataProcessingException}s).
	 *
	 * <p>The parsing parsing/writing won't stop (unless the error handler rethrows the {@link DataProcessingException} or manually stops the process).</p>
	 *
	 ...
	 */
	protected BaseCommonSettings clone(boolean clearInputSpecificSettings) {
		try {
			BaseCommonSettings out = (BaseCommonSettings) super.clone();
			if (out.format != null) {
	 ...
	 */
	@Override
	protected BaseCommonSettings clone() {
		return clone(false);
	}
