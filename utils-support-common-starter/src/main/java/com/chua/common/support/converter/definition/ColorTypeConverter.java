...
import static com.chua.common.support.constant.CommonConstant.HEX_16;
...
public class ColorTypeConverter implements TypeConverter<Color> {
    ...
    private static final Map<String, ColorName> CACHE = new LinkedHashMap<>();
    static {
        List<ColorName> colorNames = initColorList();
        for (ColorName colorName : colorNames) {
            CACHE.put(colorName.getName().toLowerCase(), colorName);
        }
    }
    /**
    ...
    private static List<ColorName> initColorList() {
        List<ColorName> colorList = new LinkedList<>();
        initColorList1(colorList);
    ...
    private static void initColorList1(List<ColorName> colorList) {
    ...
    private Color rgba(String hexStr) {
        ...
        rgb[1] = Integer.valueOf(hexStr.substring(2, 4), 16);
        rgb[2] = Integer.valueOf(hexStr.substring(4, 6), 16);
        rgb[3] = Integer.valueOf(hexStr.substring(6, 8), 16);
