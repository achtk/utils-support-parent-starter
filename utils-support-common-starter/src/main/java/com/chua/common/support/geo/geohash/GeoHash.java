...
public final class GeoHash {
     ...
     */
    public static String encodeHash(Point point, int length) {
        return encodeHash(point.latitude(), point.longitude(), length);
    }
     ...
     */
    public static String encodeHash(Point point) {
        return encodeHash(point.latitude(), point.longitude(), MAX_HASH_LENGTH);
    }
     ...
     */
    public static Point decodeHash(String geohash) {
    ...
    public static boolean hashContains(String hash, double lat, double lon) {
        Point centre = decodeHash(hash);
        return Math.abs(centre.latitude() - lat) <= heightDegrees(hash.length()) / 2
                && Math.abs(to180(centre.longitude() - lon)) <= widthDegrees(hash.length()) / 2;
    }
