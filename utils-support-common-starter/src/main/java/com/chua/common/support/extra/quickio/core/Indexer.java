...
import static com.chua.common.support.extra.quickio.core.Constants.INDEX;
        ...
        list.forEach(t -> {
            ...
            indexObjects.forEach(indexObject -> {
                ...
                if (guardMap.getOrDefault(key1, false)) {
                    throw new QuException(key1 + Constants.INDEX_ALREADY_EXISTS);
                } else {
     ...
     long getIndexId(Class<?> tClass, String fieldName, Object filedValue) {
         try {
            if (tClass.getDeclaredField(fieldName).isAnnotationPresent(Index.class)) {
                ...
                throw new QuException(Constants.NON_INDEXED_FIELD);
            }
