...
public class Img implements Serializable {
     ...
     * @return Img
     */
    public static Img from(Path imagePath) throws IOException {
     ...
     * @since 4.4.1
     */
    public static Img from(Resource resource) throws IOException {
        return from(resource.getUrl());
    }
     ...
     * @return Img
     */
    public static Img from(InputStream in) throws IOException {
     ...
     * @return Img
     */
    public static Img from(ImageInputStream imageStream) throws IOException {
     ...
     * @return Img
     */
    public static Img from(URL imageUrl) throws IOException {
    ...
    public Img scale(float scale) {
        ...
        if (ImageUtils.IMAGE_TYPE_PNG.equals(this.targetImageType)) {
            ...
            // 缩放后的图片宽
            final int width = NumberUtils.mul((Number) srcImg.getWidth(null), scale).intValue();
            final int height = NumberUtils.mul((Number) srcImg.getHeight(null), scale).intValue();
            scale(width, height);
        }
        return this;
    }
    ...
    public Img scale(int width, int height) {
        ...
        if (ImageUtils.IMAGE_TYPE_PNG.equals(this.targetImageType)) {
            ...
            final double sx = NumberUtils.div(width, srcWidth);
            final double sy = NumberUtils.div(height, srcHeight);
    ...
    public boolean write(File targetFile) throws IOException {
        final String formatName = FileUtils.getExtension(targetFile);
