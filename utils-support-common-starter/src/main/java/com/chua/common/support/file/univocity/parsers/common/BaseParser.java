...
import com.chua.common.support.file.univocity.parsers.common.iterators.BaseRecordIterator;
import com.chua.common.support.file.univocity.parsers.common.iterators.BaseRowIterator;
import com.chua.common.support.file.univocity.parsers.common.processor.RowProcessor;
import com.chua.common.support.file.univocity.parsers.common.processor.core.NoopProcessor;
...
import static com.chua.common.support.file.univocity.parsers.common.ArgumentUtils.displayLineSeparators;

/**
 * The AbstractParser class provides a common ground for all parsers in univocity-parsers.
 * <p> It handles all settings defined by {@link BaseCommonParserSettings}, and delegates the parsing algorithm implementation to its subclasses through the
 * abstract method {@link BaseParser#parseRecord()}
 ...
 * @see com.chua.common.support.file.univocity.parsers.common.input.CharInputReader
 * @see com.chua.common.support.file.univocity.parsers.common.ParserOutput
 */
@SuppressWarnings("ALL")
public abstract class BaseParser<T extends BaseCommonParserSettings<?>> {
	...
	/**
	 * All parsers must support, at the very least, the settings provided by {@link BaseCommonParserSettings}. The AbstractParser requires its configuration to be
	 * properly initialized.
	 *
	 * @param settings the parser configuration
	 */
	public BaseParser(T settings) {
	 ...
	 * {@link BaseCommonParserSettings#getRowProcessor()}.
	 *
	 ...
	 * {@link BaseParser#parseRecord()} will perform the following steps:
	 * <ul>
	 ...
	 * unless {@link BaseCommonParserSettings#isAutoClosingEnabled()} evaluates to {@code false}. The exception should contain the cause and more information about where in the input the error happened.
	 *
	 * @see com.chua.common.support.file.univocity.parsers.common.input.CharInputReader
	 * @see com.chua.common.support.file.univocity.parsers.common.input.CharAppender
	 * @see com.chua.common.support.file.univocity.parsers.common.ParserOutput
	...
	private String[] handleEof() {
		String[] row = null;
		try {
			boolean value = consumeValueOnEof();
			boolean express = output.column != 0 || (value && !context.isStopped());
			if (express) {
				if (output.appender.length() > 0 || value) {
					output.valueParsed();
	 ...
	 * The parsed records must be read one by one with the invocation of {@link BaseParser#parseNext()}.
	 * The user may invoke @link {@link BaseParser#stopParsing()} to stop reading from the input.
	 *
	 * @param reader The input to be parsed.
	 */
	public final void beginParsing(Reader reader) {
		...
		if (processor instanceof AbstractDefaultConversionProcessor) {
			AbstractDefaultConversionProcessor conversionProcessor = ((AbstractDefaultConversionProcessor) processor);
	...
	void extractHeadersIfRequired() {
		while (extractHeaders && output.parsedHeaders == null && !context.isStopped() && !extractingHeaders) {
			...
			try {
				//disables any users provided processors to capture headers
	...
	/**
	 * Parses the next record from the input. Note that {@link BaseParser#beginParsing(Reader)} must have been invoked once before calling this method.
	 ...
	 * unless {@link BaseCommonParserSettings#isAutoClosingEnabled()} evaluates to {@code false}.
	 *
	 ...
	 * {@link BaseCommonParserSettings#getRowProcessor()}.
	 *
	 * @param input The input to be parsed. The input stream will be closed automatically, unless {@link BaseCommonParserSettings#isAutoClosingEnabled()} evaluates
	 *              to {@code false}.
	 */
	 ...
	 * {@link BaseCommonParserSettings#getRowProcessor()}.
	 *
	 * @param input    The input to be parsed. The input stream will be closed automatically, unless {@link BaseCommonParserSettings#isAutoClosingEnabled()}
	 ...
	 *              {@link              BaseCommonParserSettings#isAutoClosingEnabled()} evaluates to {@code false}.
	 */
	...
	/**
	 * Returns the metadata associated with {@link Record}s parsed from the input using {@link BaseParser#parseAllRecords(File)} or
	 * {@link BaseParser#parseNextRecord()}.
	 *
	...
	public final IterableResult<String[], ParsingContext> iterate(final File input, final Charset encoding) {
		return new BaseRowIterator(this) {
			@Override
