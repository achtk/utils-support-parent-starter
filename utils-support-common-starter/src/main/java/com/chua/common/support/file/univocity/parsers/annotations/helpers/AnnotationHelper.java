...
import com.chua.common.support.file.univocity.parsers.annotations.*;
...
import com.chua.common.support.file.univocity.parsers.common.beans.BeanHelper;
...
public class AnnotationHelper {
    ...
    /**
     * Identifies the proper conversion for a given type and an annotation from the package {@link com.chua.common.support.file.univocity.parsers.annotations}
     *
     * @param classType  the type to have conversions applied to
     * @param annotation the annotation from {@link com.chua.common.support.file.univocity.parsers.annotations} that identifies a {@link Conversion} instance.
    ...
    public static Conversion getConversion(Class fieldType, AnnotatedElement target, Annotation annotation, String nullRead, String nullWrite) {
        try {
            ...
            if (annType == NullString.class) {
                ...
                boolean express = Number.class.isAssignableFrom(fieldType) || (fieldType.isPrimitive()) && fieldType != boolean.class && fieldType != char.class;
            ...
            boolean express = fieldType == String.class && (nullRead != null || nullWrite != null);
            if (express) {
     ...
     * @param parsed    the {@link Parsed} annotation from {@link com.chua.common.support.file.univocity.parsers.annotations}.
    ...
    private static boolean allFieldsIndexOrNameBased(boolean searchName, Class<?> beanClass, MethodFilter filter) {
        ...
        for (TransformedHeader header : getFieldSequence(beanClass, true, null, filter)) {
            ...
            if (annotation != null) {
                ...
                boolean express = (index != -1 && searchName) || (index == -1 && !searchName);
                if (express) {
                    return false;
                }
            }
    ...
    public static String[] deriveHeaderNamesFromFields(Class<?> beanClass, MethodFilter filter) {
        ...
        for (TransformedHeader field : sequence) {
            if (field == null) {
                // some field has an index that goes beyond list of header names, can't derive.
    ...
    public static <A extends Annotation> List<Method> getAnnotatedMethods(Class<?> beanClass, MethodFilter filter, Class<A> annotationType) {
        ...
        do {
            ...
            for (Method method : declared) {
                if (!method.isSynthetic() && annotationType == NoAnnotations.class) {
                    ...
                    for (Annotation annotation : annotations) {
                        boolean express = (annotationType == null && isCustomAnnotation(annotation)) || annotationType == annotation.annotationType();
                        if (express) {
                            if (filter.reject(method)) {
                                continue outer;
                            }
    ...
    public static List<Method> getAllMethods(Class<?> beanClass, MethodFilter filter) {
        return getAnnotatedMethods(beanClass, filter, NoAnnotations.class);
    }
    ...
    private static boolean isCustomAnnotation(Annotation annotation) {
        ...
        if (CUSTOM_ANNOTATION_TYPES.contains(annotationType)) {
            return true;
        }
        ...
        if (annotationType.getName().startsWith(JAVA_ANNOTATION)) {
            ...
            CUSTOM_ANNOTATION_TYPES.add(annotationType);
            return true;
        }
    }
    ...
    private static final class NoAnnotations implements Annotation {
        @Override
        public Class<? extends Annotation> annotationType() {
            return NoAnnotations.class;
        }
    }
}
