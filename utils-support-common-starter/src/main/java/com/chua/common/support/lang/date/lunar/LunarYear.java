...
public class LunarYear {
    ...
    private void compute() {
        ...
        if (null == currentYearLeap) {
            currentYearLeap = -1;
            if (hs[step13] <= jq[step24]) {
                int i = 1;
                while (hs[i + 1] > jq[step2 * i] && i < step13) {
                    i++;
                }
                currentYearLeap = i;
            }
        ...
        for (int i = 0, j = dayCounts.length; i < j; i++) {
            ...
            this.months.add(new LunarMonth(y, cm, dayCounts[i], hs[i] + SolarTime.J2000));
            if (!isNextLeap) {
                m++;
            }
            if (m == 13) {
    ...
    public String getZhiShui() {
        int offset = 4 - SolarTime.fromJulianDay(getMonth(1).getFirstJulianDay()).getLunar().getDayZhiIndex();
        if (offset < 0) {
            offset += 12;
        }
    ...
    public String getFenBing() {
        int offset = 2 - SolarTime.fromJulianDay(getMonth(1).getFirstJulianDay()).getLunar().getDayGanIndex();
        if (offset < 0) {
            offset += 10;
        }
    ...
    public String getGengTian() {
        int offset = 1 - SolarTime.fromJulianDay(getMonth(1).getFirstJulianDay()).getLunar().getDayZhiIndex();
        if (offset < 0) {
            offset += 12;
        }
    ...
    public String getDeJin() {
        int offset = 7 - SolarTime.fromJulianDay(getMonth(1).getFirstJulianDay()).getLunar().getDayGanIndex();
        if (offset < 0) {
            offset += 10;
        }
