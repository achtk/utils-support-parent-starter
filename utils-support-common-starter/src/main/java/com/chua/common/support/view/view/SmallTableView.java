 ...
 * @author CH
 */
public class SmallTableView implements View {
    ...
    private Table table;
    private List<List> content = new LinkedList<>();

    public SmallTableView(String[] header, Integer maxWidth, Integer maxLength) {
        this.content.add(Arrays.asList(header));
        this.maxLength = maxLength;
        this.maxWidth = maxWidth;
    }


    public SmallTableView(String... header) {
        if (header.length > 0) {
            this.content.add(Arrays.asList(header));
        }
        this.maxLength = 10;
     ...
     */
    public SmallTableView addRow(Object... columnDataArray) {
        if (null == columnDataArray) {
            return this;
        }

        this.content.add(Arrays.asList(columnDataArray));
        return this;
    }
    ...
    private Table buildTable(List content) {
     ...
     * @return
     */
    public String printTable(String... symbols) {
        ...
        //打印首行分割符号
        stringBuilder.append(StringUtils.repeat("-", totalColSize));
        formatTable.getContent()
                .forEach(row -> {
                    stringBuilder.append("\r\n");
                    row.forEach(stringBuilder::append);
                    stringBuilder.append("\r\n");
                    //打印每行分割符号
                    stringBuilder.append(StringUtils.repeat("-", totalColSize));
                });
        ...
        List<List<String>> formatList = table.getContent().stream().map(
                row -> {
                    ...
                    return row.stream().map(cell -> {
                        ...
                        int cellSize = originMaxWidthList.get(j) + symbolLen - StringUtils.getCharCount(cell);
                ...
                .map(rows -> Arrays.stream(rows)
                        .mapToInt(s -> {
                            ...
                            if (s == null) {
                                ...
                                return s.length() + StringUtils.getCharCount(s);
                            }
                        }).max().orElse(0)
                ).collect(Collectors.toList());
    }
