 ...
 * @date 2023/08/31
 */
public abstract class BasePathToken {

    private static final String SYMBOL = "[-1]";
    private BasePathToken prev;
    private BasePathToken next;
    ...
    BasePathToken appendTailToken(BasePathToken next) {
        this.next = next;
        this.next.prev = this;
        return next;
    }
        ...
        if (properties.size() == 1) {
            ...
            if (propertyVal == JsonProvider.UNDEFINED) {
                ...
                if (isLeaf()) {
                    ...
                    boolean b = !(isUpstreamDefinite() && isTokenDefinite()) &&
                            !ctx.options().contains(Option.REQUIRE_PROPERTIES) ||
     ...
     * @param next next
     */
    public void setNext(final BasePathToken next) {
        this.next = next;
    }
