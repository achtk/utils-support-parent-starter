...
import com.chua.common.support.file.univocity.parsers.common.processor.*;
import com.chua.common.support.file.univocity.parsers.common.processor.core.*;
import com.chua.common.support.file.univocity.parsers.common.processor.core.AbstractBeanProcessorAbstract;
...
/**
 * This is the parent class for all configuration classes used by parsers ({@link BaseParser})
 *
 * <p>By default, all parsers work with, at least, the following configuration options in addition to the ones provided by {@link BaseCommonSettings}:
 *
 * <ul>
 ...
 * <li><b>columnReorderingEnabled <i>(defaults to true)</i>:</b> indicates whether fields selected using the field selection methods (defined by the parent class {@link BaseCommonSettings}) should be reordered.
 ...
 */
public abstract class BaseCommonParserSettings<F extends BaseFormat> extends BaseCommonSettings<F> {
     ...
     * @see BaseMasterDetailListProcessor
     ...
     * @see AbstractObjectListProcessorAbstract
     * @see AbstractMasterDetailProcessorAbstract
     * @see AbstractMasterDetailListProcessorAbstract
    ...
    /**
     * Indicates whether fields selected using the field selection methods (defined by the parent class {@link BaseCommonSettings}) should be reordered (defaults to
     * true).
    ...
    /**
     * Defines whether fields selected using the field selection methods (defined by the parent class {@link BaseCommonSettings}) should be reordered (defaults to
     * true).
     * <p>When disabled, each parsed record will contain values for all columns, in the order they occur in the input. Fields which were not selected will not
    ...
    final void runAutomaticConfiguration() {
        ...
        if (processor instanceof AbstractBeanProcessorAbstract<?, ?>) {
            beanClass = ((AbstractBeanProcessorAbstract<?, ?>) processor).getBeanClass();
    ...
    @Override
    protected BaseCommonParserSettings clone(boolean clearInputSpecificSettings) {
        return (BaseCommonParserSettings) super.clone(clearInputSpecificSettings);
    }

    @Override
    protected BaseCommonParserSettings clone() {
        return (BaseCommonParserSettings) super.clone();
    }
